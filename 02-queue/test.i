...
static inline int dequeue(struct queuelist *qh, struct sendmsg *nd)
{
 ...
 if (qh->cur == 0) {
  ...
  qh->cur--;
  nd = ({ const typeof( ((struct sendmsg *)0)->node ) *__mptr = (tmp); (struct sendmsg *)( (char *)__mptr - ((size_t) &((struct sendmsg *)0)->node) );});
  printf("%s %d, nd %p %d, p.c.n %p %p %p\n", __func__, 51,
    &nd->node, nd->id, qh->head.prev, &qh->head, qh->head.next);
  list_del(tmp);
 }
...
# 81 "test.c"
 ...
 do {
  ...
  tmp = ({ struct ulist_head *tmp = (qh)->head.prev; typeof(struct sendmsg) *nd = 
# 89 "test.c" 3 4
       ((void *)0)
# 89 "test.c"
       ; if ((qh)->cur == 0) { printf("%s %d, queue empty\n", __func__, 89); nd; } (qh)->cur--; nd = ({ const typeof( ((typeof(struct sendmsg) *)0)->node ) *__mptr = (tmp); (typeof(struct sendmsg) *)( (char *)__mptr - ((size_t) &((typeof(struct sendmsg) *)0)->node) );}); printf("%s %d, nd %p %d, p.c.n %p %p %p\n", __func__, 89, nd->node, nd->id, qh->head.prev, &qh->head, qh->head.next); list_del(tmp); ({ const typeof( ((typeof(struct sendmsg) *)0)->node ) *__mptr = (tmp); (typeof(struct sendmsg) *)( (char *)__mptr - ((size_t) &((typeof(struct sendmsg) *)0)->node) );}); });
  if (tmp)
   free(tmp);
  tmp = 
# 92 "test.c" 3 4
